            ...
            if game.log_file["file"] is not None:
                ...
                for line in file:
                    if game.log_file["connect"] in line:
                        parsed_name = line[game.log_file["splice_join_start"]:game.log_file["splice_join_end"]]
                        if not parsed_name in connected_players:
                            connected_players.append(parsed_name)
                    elif game.log_file["disconnect"] in line:
                        parsed_name = line[game.log_file["splice_left_start"]:game.log_file["splice_left_end"]]
...
if __name__ == "__main__":
    try:
      ...
      app.run(ssl_context=('/etc/letsencrypt/archive/games.caramellatte.dev/fullchain3.pem', '/etc/letsencrypt/archive/games.caramellatte.dev/privkey3.pem'), host="0.0.0.0", port=8080)
    except:
